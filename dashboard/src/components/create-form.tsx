...
const Stepper = ({
  ...
  return (
    <div className={cx(
      "flex",
      config.orientation === "vertical" ? "flex-col space-y-2" : "space-x-4"
    )}>
...
export function createForm(
  ...
  const methods = useForm<Record<string, any>>({
    resolver: zodResolver(
      Array.isArray(schemaOrSections)
        ? z.object(schemaOrSections.reduce((acc, section) => ({
          ...acc,
  ...
  function FormComponent(props: React.FormHTMLAttributes<HTMLFormElement>) {
    ...
    const handleSubmit = async (values: any) => {
      ...
      try {
        const transformedValues = config?.transformValues
        ...
        <FormField
          ...
          render={({ field, fieldState }) => (
            <FormItem className={finalConfig.containerClassName}>
              {value._def.label && (
                <FormLabel>{value._def.label}</FormLabel>
              )}
              <FormControl>
    ...
    const renderInput = (def: any, field: any, config?: ComponentConfig) => {
      ...
      console.log("Type:", typeName, "Final:", finalTypeName, "Label:", label, "InputType:", inputType ? inputType : "none", finalDef.schema?._def.label);
      ...
      if (config?.layout?.type === "stepper") {
        return (
          <div className="space-y-8">
            ...
            />
            <div className="mt-8">
              {sections[activeStep] && Object.entries(sections[activeStep].schema.shape)
                .map(([key, value]) => renderField(key, value, config.layout?.components?.[key]))}
            </div>
          </div>
        );
      }
